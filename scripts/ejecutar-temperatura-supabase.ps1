# Ejecuta la migracion de temperatura en Supabase
param(
  [string]$AccessToken = '',
...
$sqlPath = Join-Path $root 'supabase\migrations\20250616_temperature_monitoring.sql'
$secretsPath = Join-Path $root 'data\supabase-admin.local.json'

function Test-TempTables {
  param([string]$Key)
  $headers = @{
    apikey       = $Key
  ...
  try {
    $null = Invoke-RestMethod -Uri "https://$ProjectRef.supabase.co/rest/v1/temp_areas?select=id&limit=1" -Headers $headers -Method Get
    return $true
  } catch {
    return $false
  }
...
Write-Host ''
Write-Host '=== Monitoreo de Temperatura — Supabase ===' -ForegroundColor Cyan
Write-Host ''

if (Test-TempTables -Key $AnonKey) {
  Write-Host 'OK — Las tablas de temperatura ya existen.' -ForegroundColor Green
  exit 0
}
...
if ($token) {
  ...
  try {
    ...
    Write-Host 'AVISO — SQL enviado pero temp_areas aun no responde. Revise el SQL Editor.' -ForegroundColor Yellow
  } catch {
...
for ($i = 1; $i -le $max; $i++) {
  ...
  if (Test-TempTables -Key $AnonKey) {
    Write-Host 'OK — Tablas detectadas. Temperatura lista para usar.' -ForegroundColor Green
    exit 0
  }
  Write-Host "Esperando migracion... ($i/$max)"
}

Write-Host 'Tiempo agotado. Ejecute el SQL en Supabase y recargue temperatura.html con Ctrl+F5.' -ForegroundColor Yellow
exit 1
